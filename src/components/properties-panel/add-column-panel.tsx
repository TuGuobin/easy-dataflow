    ...
    <BasePanel node={node}>
      {({ themeConfig }) => (
        <div className={`mb-5 ${themeConfig.text}`}>
            ...
            items={(node.data.newColumns || []).map((column, index) => ({
              ...
              content: (themeConfig) => (
                <div className="text-xs">
                  ...
                  <span className="text-gray-400 ml-2">默认值: {String(column.defaultValue)}</span>
                </div>
              ),
          ...
          {!isEditing ? (
            <ActionButton onClick={() => setIsEditing(true)} themeConfig={themeConfig} icon="fa-plus" text="添加新列" className="mb-2" />
          ) : (
            <EditForm
              title="新列配置"
              ...
              confirmText="添加"
            >
              <Input label="列名" themeConfig={themeConfig} type="text" value={newColumn.name} onChange={(e) => setNewColumn((prev) => ({ ...prev, name: e.target.value }))} className="flex-1" />
              <Select
                label="数据类型"
                themeConfig={themeConfig}
                ...
                options={Object.entries(COL_TYPES).map(([key, value]) => ({ value: key, label: value }))}
                className="flex-1"
              />
              <Input label="默认值" themeConfig={themeConfig} type="text" value={String(newColumn.defaultValue)} onChange={(e) => handleDefaultValueChange(e.target.value)} className="flex-1" />
            </EditForm>
          )}
        </div>
      )}
